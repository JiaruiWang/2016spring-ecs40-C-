...
Plane::Plane(int flightNumber) :reserved( 0 ), flightNumber(flightNumber)
{
}  // Plane()
...
ifstream& operator>>(ifstream& inf, Plane& plane)
{
  ...
  for(row = 0; row < plane.rows; row++)
    plane.passengers[row].initialize(plane.width); // for each row
  ...
  while(inf2.read((char*) &passenger, sizeof(Passenger)))
  {
    if(passenger == plane.flightNumber)
    {
      plane.passengers[passenger.row - Plane::FIRST_ROW]
        ...
        // cout << plane.passengers[passenger.row - Plane::FIRST_ROW][passenger.seat - Plane::FIRST_SEAT] << " ";
        // cout << passenger.flightNum<< " " << passenger.row<< " " << passenger.seat<< " " << passenger.name << endl;
      plane.reserved++;
...
void Plane::addFlight()
{
  ...
  for(int row = 0; row < rows; row++)
    passengers[row].initialize(width); // for each row
}  // addFlight()
...
Plane& Plane::operator--(int dummy)
{
  ...
  for(int row = 0; row < plane.rows; row++)
    for(int seatNum = 0; seatNum < plane.width; seatNum++)
      ...
      { 
        ...
        os << passenger << endl;
        // cout << plane.flightNumber << " " << row << " " << seatNum << endl;
      }  // if not EM
  ...
  for(int row = 0; row < rows; row++)
    for(int seatNum = 0; seatNum < width; seatNum++)
      if(passengers[row][seatNum] != LinkedList::EMPTY)
      {
        inf.seekg(passengers[row][seatNum], ios::beg);
        inf.read((char*) &passenger, sizeof(Passenger));
        // cout << "found ?= true in plane.cpp\n";
        // cout <<"1passenger:" <<passenger.name << " name:" << name <<endl;
        if(passenger == name)
        {
          cout << "Flight #" << flightNumber;
          cout << " Row: " << row + Plane::FIRST_ROW;
          cout << " Seat: " << static_cast<char>(seatNum + Plane::FIRST_SEAT);
          cout << endl;
          found = true;
          // cout << "found == true in plane.cpp\n";
        }  // if found name
