...
testerDirectory="/home/wjr/40spring/p8/Testing"
...
if ! grep "find *(.*BSTNode<T> *\*.*,.*T.*)" BinarySearchTree.cpp >& /dev/null ; then
  echo 'recursive find not found in BinarySearchTree.cpp' >> designResults.txt
fi

if ! grep "inOrder *(.*BSTNode<T> *\*.*)" BinarySearchTree.cpp >& /dev/null ; then
  echo 'recursive inOrder not found in BinarySearchTree.cpp' >> designResultls.txt
fi

if ! grep "findMin *(.*BSTNode<T> *\*.*)" BinarySearchTree.cpp >& /dev/null ; then
  echo 'recursive findMin not found in BinarySearchTree.cpp' >> designResults.txt
fi

if ! grep "makeEmpty *(.*BSTNode<T> *\*.*)" BinarySearchTree.cpp >& /dev/null ; then
  echo 'recursive makeEmpty not found in BinarySearchTree.cpp' >> designResults.txt
fi

if ! grep "postOrder *(.*BSTNode<T> *\*.*)" BinarySearchTree.cpp >& /dev/null ; then
  echo 'recursive postOrder not found in BinarySearchTree.cpp' >> designResults.txt
fi

if ! grep "find *( *const *BSTNode" BinarySearchTree.h >& /dev/null ; then
  echo 'Missing const BSTNode in recursive find in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "find *(.*BSTNode.*, *const" BinarySearchTree.h >& /dev/null ; then
  echo 'Missing const T in recursive find in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "find.*BSTNode.*) *const" BinarySearchTree.h >& /dev/null ; then
  echo 'Recursive find not a const function in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "const .*findMin" BinarySearchTree.h >& /dev/null ; then
  echo 'findMin does not return a const BSTNode* in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "findMin *( *const *BSTNode" BinarySearchTree.h >& /dev/null ; then
  echo 'Missing const BSTNode in findMin in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "findMin.*) *const" BinarySearchTree.h >& /dev/null ; then
  echo 'findMin is not a const function in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "inOrder.*const *BSTNode" BinarySearchTree.h >& /dev/null ; then
  echo 'Missing const BSTNode in recursive inOrder() in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "inOrder.*BSTNode.*) *const" BinarySearchTree.h >& /dev/null ; then
  echo 'Recursive inOrder() is not a const function in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "inOrder.*( *) *const" BinarySearchTree.h >& /dev/null ; then
  echo 'Non-recursive inOrder() is not a const function in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "postOrder.*const *BSTNode" BinarySearchTree.h >& /dev/null ; then
  echo 'Missing const BSTNode in recursive postOrder() in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "postOrder.*BSTNode.*) *const" BinarySearchTree.h >& /dev/null ; then
  echo 'Recursive postOrder() is not a const function in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "postOrder.*( *) *const" BinarySearchTree.h >& /dev/null ; then
  echo 'Non-recursive postOrder() is not a const function in BinarySearchTree.h' >> designResults.txt
fi

if ! grep "insert.*( *const *T" BinarySearchTree.h >& /dev/null ; then
  echo 'Missing const T in non-recursive insert() in BinarySearchTree.h' >> designResults.txt
fi
